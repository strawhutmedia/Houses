...
| **Probate / Trust Sales** | `probateMLS.js` | ✅ **Live** | MLS listings (via Redfin's public GIS API) whose remarks say "probate", "trust sale", "estate sale", "court confirmation", etc. — estate-motivated sellers in the family's target areas (SGV + NE LA/Glendale/Burbank + Portland). The API's `keyword` param is a no-op, so we tile bounding boxes and grep remarks client-side; saturated boxes (350-row cap) split into quadrants. Keywords via `ES_PROBATE_KEYWORDS`. |
